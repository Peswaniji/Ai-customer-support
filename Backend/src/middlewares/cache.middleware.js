...
export const cache =
  (ttl = 60) =>
  async (req, res, next) => {
    // FIX: isOpen is now a boolean property (not a method call)
...
export const invalidateCache = async (businessId) => {
  ...
  try {
    const keys = await redisClient.scan(`cache:*:${businessId}:*`);
    if (keys.length > 0) {
      await redisClient.del(keys);
      console.log(`Cache invalidated for business ${businessId}: ${keys.length} keys`);
    }
  } catch (err) {
